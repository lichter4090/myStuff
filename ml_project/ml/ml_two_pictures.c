...
int main(int argc, char** argv)
{
	...
	size_t i = 0, epochs_per_frame = 103, batches_per_frame = 28, batch_size = 28, barch_count = (data->rows + batch_size - 1) / batch_size, current_batch;
	float rate = 1;

	plot* cost_plot = plotAlloc();

	Mat* input_batch = matAlloc(data->rows, input_size);
	Mat* output_batch = matAlloc(data->rows, output_size);
	...
	while (!WindowShouldClose() && i < MAX_ITERATION)
	{
		if (i < MAX_ITERATION)
		{
			...
			for (size_t j = 0; j < epochs_per_frame && i < MAX_ITERATION && !paused; j++)
			{
				current_batch = i % barch_count;

				input_batch->es = &MAT_AT(input, current_batch * batch_size, 0);
				output_batch->es = &MAT_AT(output, current_batch * batch_size, 0);
				
				nnBackProp(network, gradient, input_batch, output_batch);
	...
	int out_width = 512;
	int out_height = 512;
	uint8_t* out_pixels = (uint8_t*)malloc(sizeof(uint8_t) * out_width * out_height);
